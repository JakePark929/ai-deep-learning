...
def prompt_and_generate(query, docs, price):
    prompt = f"""아래 질문을 기반으로 검색된 질문에 첨부된 뉴스를 참고하여 질문에 대한 답변을 생성하시오. 질문과 가장 적절한 뉴스에서 등장한 이벤트까지 출력하시오.
...
def first_chain(query):
    prompt = """아래 질문에 기업명이 포한되어 있다면, 기업번호(ticker)를 출력하시오. 반드시 ticker만을 출력하고, 기업명이 포함되어 있지 않은 경우는 0을 출력하시오.
    출력 예시: {'기업명': 기업번호}\n"""
    prompt += f"질문: {query}"
    answer = chatgpt_generate(prompt)
